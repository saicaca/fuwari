...
function isFootnoteParagraph(node) {
  ...
  const a = node.children?.find?.((c) => c.type === "element" && c.tagName === "a" && typeof c.properties?.id === "string");
  if (!a) return false;
...
export function rehypeTypstFootnotes() {
  return function transformer(tree) {
    try { console.log("[rehype-typst-footnotes] run transformer"); } catch {}
    let pLogged = 0;
    visit(tree, (node, index, parent) => {
      if (!parent || typeof index !== "number") return;
      if (node.type === "element" && node.tagName === "p") {
        const textFull = (node.children || []).map((c) => (c.type === "text" ? c.value : c.tagName ? `<${c.tagName}>` : "")).join("");
        if (/Footnotes/i.test(textFull)) {
          try { console.log("[rehype-typst-footnotes] p contains Footnotes:", textFull.slice(0, 160)); } catch {}
        } else if (pLogged < 3) {
          try { console.log("[rehype-typst-footnotes] p:", textFull.slice(0, 80)); } catch {}
          pLogged++;
        }
      }
      // Direct match on a paragraph that begins with "Footnotes:" pattern
      if (isFootnoteParagraph(node)) {
        // Existing logic handles a single footnote line
      } else if (node.type === "element" && node.tagName === "a" && typeof node.properties?.id === "string" && /^fn-?\d+$/i.test(node.properties.id)) {
        ...
        if (!p) return;
        // Re-target the vars for code below
        node = p; index = parent.children.indexOf(p); parent = parent.parent || parent; // parent.parent is not in HAST, so keep parent
        if (!parent || typeof index !== "number") return;
      } else {
        return;
      }
      ...
      const aIdx = children.findIndex((c) => c.type === "element" && c.tagName === "a" && /^fn-?\d+$/i.test(String(c.properties?.id || "")));
      if (aIdx < 0) return;
      ...
      try {
        ...
        console.log("[rehype-typst-footnotes] transformed footnotes for id", a.properties.id);
      } catch (e) {
